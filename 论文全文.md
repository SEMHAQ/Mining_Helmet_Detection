...
井下安全帽佩戴检测面临低光照、高粉尘、小目标和遮挡等多重挑战。现有检测算法在多尺度特征融合与前景定位精度方面存在不足。为此，本文以YOLOv11n为基础，提出ECB-YOLO检测模型。模型在骨干网络引入EMSCP模块，通过分组多分支卷积实现多尺度上下文信息的动态捕捉；颈部网络采用BiFPN双向加权特征融合结构，自适应分配不同尺度特征的融合权重；检测头前端集成ContextGuided模块，通过局部-周围-全局三分支建模优化前景区域特征。在CUMT-HelmeT数据集上，ECB-YOLO的mAP50达到86.63%，精确率89.71%，召回率78.59%，较基线YOLOv11n分别提升3.48、5.70和0.69个百分点。模型参数量12.5M，推理速度97.2 FPS。
...
井下场景的视觉目标检测是矿山智能化建设的重要方向。针对低光照和高粉尘导致的图像质量下降，已有工作主要从图像预处理和网络结构优化两个方向着手。Qiu等[7]提出IDOD-YOLOv7模型，通过图像去雾预处理与检测网络的联合优化，提升了低光照雾天环境下的目标检测精度。Lin[8]针对安全帽检测任务改进YOLOv8结构，在保持轻量化的同时提升了小目标的检测性能。Tao等[9]基于感受野注意力机制和多尺度特征融合增强YOLO的特征提取能力，在工业小目标检测中表现尚可。YOLOv10[10]通过端到端检测架构消除了NMS后处理，实现了检测精度与推理速度的有效平衡。韩昊等[19]将改进YOLOv8应用于矿山井下钻头检测，验证了YOLO系列在井下视觉任务中的适用性。

近年来，YOLO系列在矿山安全检测中的应用持续深化：张等[22]针对矿山小目标检测改进YOLOv8的特征金字塔结构，提升了远距离目标的检出率；Li等[23]将注意力机制与YOLOv8结合用于矿山人员检测，在遮挡场景下有一定改善。但一个共性问题是：多数方案只解决单一环境挑战，很少兼顾低光照、遮挡和小目标的叠加影响。部分方案虽然精度较高，却以牺牲推理速度为代价[20]。如何在保持轻量性的同时提升检测鲁棒性，仍值得进一步探索。
...
多尺度特征融合直接影响目标检测的精度。PAN-FPN通过自顶向下和自底向上的路径实现跨尺度特征传递，但采用等权融合策略，无法区分不同尺度特征的重要性[11]。BiFPN在此基础上引入可学习的融合权重，通过快速归一化机制自适应分配不同尺度特征的贡献度，在EfficientDet中取得了显著效果。ASFF（Adaptive Spatial Feature Fusion）通过空间注意力机制实现特征图的自适应融合，但增加了额外的计算开销。

上下文建模方面，SE（Squeeze-and-Excitation）模块通过全局平均池化学习通道注意力权重，CBAM进一步结合空间注意力机制，实现了通道和空间维度的双重关注。CGNet（Context Guided Network）提出局部-周围-全局的三分支结构，在语义分割任务中有效利用了上下文信息。近年来，轻量化注意力机制受到广泛关注：ShuffleNet V2[24]通过通道shuffle操作实现跨组信息交换，在极低计算开销下保持了合理的特征表达能力；ECA-Net[25]通过一维卷积替代全连接层学习通道注意力，避免了SE模块中的降维操作，参数量仅为SE的1/10。在目标检测领域，SPD-Conv[26]提出空间到深度卷积替代池化操作，在小目标检测中有效保留了空间信息。上述方法在通用视觉任务中表现良好，但针对井下低光照、高粉尘等特殊场景的适配性仍有不足。
...
多尺度特征融合的质量对检测性能有较大影响。原始PAN-FPN采用简单跳层连接与等权融合，无法自适应区分不同特征的重要性。我们采用BiFPN结构替换PAN-FPN（见图3），改进涉及三个方面。连接拓扑上，保留自顶向下和自底向上的融合路径，同时增加同一尺度特征的跳跃连接以减少信息传递损失。融合机制上，针对不同分辨率的输入特征图引入可学习的权重参数，根据特征对检测任务的贡献度动态分配融合权重。分辨率匹配上，通过上采样和下采样统一不同尺度特征图的分辨率与通道数，确保融合的有效性。
...
试验采用CUMT-HelmeT公开井下安全帽数据集[12]，该数据集采集自真实煤矿井下场景，涵盖低光照、遮挡、强反光、小目标等典型工况。原始数据集包含2482张图像，标注涵盖person、helmet、no-helmet、damaged-helmet等4个主要类别。针对样本量有限和类别分布不均衡的问题，采用随机裁剪、旋转、镜像翻转、亮度调整、噪声叠加等数据增强策略，将数据集扩充至5倍，最终得到12410张标注图像。数据集按7:2:1划分为训练集、验证集和测试集。

需要指出的是，CUMT-HelmeT数据集的原始规模有限（2482张），且采集自单一煤矿场景，对不同矿种、不同地理区域和不同监控设备条件的覆盖尚不充分。数据增强缓解了样本量不足的问题，但增强样本与真实分布之间仍可能存在偏差。我们的试验结论主要适用于与该数据集类似的煤矿井下场景，模型在其他类型矿山（如金属矿、非金属矿）中的泛化性能有待验证。
...
进一步分析ECB-YOLO在各类别上的检测性能，结果见表3。person类的AP最高（93.9%），目标尺寸较大、特征显著，检测难度最低。helmet类的AP次之（92.2%），表明模型对安全帽的正类检测能力较强。no-helmet类的AP为89.2%，漏检主要集中在严重遮挡和小目标场景。damaged-helmet类的AP最低（87.9%），破损安全帽的形态多样性导致特征学习困难，是后续改进的重点方向。
...
|:-----|:------:|:---------:|:------:|
| person | 0.939 | 0.892 | 0.88 |
| helmet | 0.922 | 0.869 | 0.87 |
| no-helmet | 0.892 | 0.846 | 0.76 |
| damaged-helmet | 0.879 | 0.822 | 0.74 |

注：AP和Precision为PR曲线数据，Recall为混淆矩阵对角线值。
...
ECB-YOLO在验证集上的全部类别mAP50为0.909，较YOLOv11n（0.894）提升1.5个百分点¹。从各类别AP来看，ECB-YOLO在person类达到0.939（+0.7pp），helmet类达到0.922（+1.9pp），no-helmet类达到0.892（+0.8pp），damaged-helmet类达到0.879（+3.3pp）。damaged-helmet类的提升幅度最大，说明ContextGuided模块的上下文加权机制对形态多样的破损安全帽具有较好的适应性。

注¹：PR曲线AP基于验证集默认参数；3.4节mAP50（86.63%）基于测试集特定置信度阈值和NMS参数。
...
ECB-YOLO在各类别的正确分类率均高于YOLOv11n：person类88%（+4pp），helmet类87%（+5pp），no-helmet类76%（+5pp），damaged-helmet类74%（+6pp）。damaged-helmet类的提升幅度最大，与PR曲线分析一致，进一步验证了ContextGuided模块对困难样本的改善效果。

从误分类模式来看，两个模型的主要混淆来源相似：no-helmet类和damaged-helmet类容易被误判为background（ECB-YOLO分别为8%和10%，YOLO11n为10%和14%），说明在低光照和严重遮挡条件下，模型对这些类别的特征响应仍然不足。no-helmet类与helmet类之间也存在一定的互混现象（ECB-YOLO为6%，YOLO11n为7%），可能是由于部分未佩戴安全帽的人员手持安全帽导致的视觉相似性。
...
（4）类别不均衡。数据集中各类别样本数量差异较大，damaged-helmet等少数类别的检测性能（AP 87.9%）明显低于多数类别，类别不均衡问题的处理策略有待深入研究。
...
（1）CUMT-HelmeT数据集上的三次独立试验结果显示，ECB-YOLO的mAP50为86.63%±0.18%，精确率89.71%±0.19%，较基线分别提升3.48和5.70个百分点（$p < 0.001$）。与SSD、Faster R-CNN、YOLOv8n、YOLOv9等8个模型的对比中，ECB-YOLO在精度和速度上均有一定优势。从类别维度看，person类AP最高（93.9%），damaged-helmet类最低（87.9%），二者之间的差距提示形态多样性仍是当前模型的一个薄弱环节。
...
需要指出的是，ECB-YOLO的召回率为78.59%，在安全帽检测这类安全关键应用中，约21%的漏检率意味着部分未佩戴安全帽的作业人员可能未被识别。漏检主要集中于严重遮挡和远距离小目标两类场景。在实际部署中，可通过降低置信度阈值（召回率可提升至约85%）和视频流多帧融合策略进行补偿。我们的试验基于单一数据集（CUMT-HelmeT），模型在其他矿山场景中的泛化性能有待验证。详细的局限性分析见3.10节。
